...
class SiPixelDigisCUDA : public PortableCUDADeviceCollection_2layouts<SiPixelDigisCUDASOA_H_D,
                                                                      SiPixelDigisCUDASOA_DO,
                                                                      SiPixelDigisCUDASOAView,
                                                                      SiPixelDigisCUDASOAConstView> {
public:
  using PortableCUDADeviceCollection_2layouts<SiPixelDigisCUDASOA_H_D,
                                              SiPixelDigisCUDASOA_DO,
                                              SiPixelDigisCUDASOAView,
                                              SiPixelDigisCUDASOAConstView>::PortableCUDADeviceCollection_2layouts;
  ...
  cms::cuda::host::unique_ptr<std::byte[]> copyAllToHostAsync(cudaStream_t stream) const {
    ...
    cudaCheck(cudaMemcpyAsync(
        ret.get(), layout0().metadata().data(), layout0().metadata().byteSize(), cudaMemcpyDeviceToHost, stream));
    return ret;
  }
