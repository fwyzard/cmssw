...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  class Analyzer : public edm::global::EDAnalyzer<> {
  public:
    explicit Analyzer(edm::ParameterSet const& config);
  ...
  Analyzer::Analyzer(edm::ParameterSet const& config)
  ...
  void Analyzer::analyze(edm::StreamID,
    ...
    for (auto const& device : cms::alpakatools::devices<Platform>()) {
      ...
      test::wrapper(queue, in1_d.data(), in2_d.data(), out_d.data(), size_);
...
DEFINE_FWK_ALPAKA_MODULE(Analyzer);
