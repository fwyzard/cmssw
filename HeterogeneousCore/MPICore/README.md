...
`MPISource`, it transmits to it all EDM run, lumi and event transitions, and instructs the `MPISource` to replicate
...
For each event, it first sends a metadata message describing the products to be transferred, including their number
...
regions; otherwise, the product is serialised into a single buffer using its ROOT dictionary. The regions and the
buffer are sent to another process over the MPI communication channel.
...
Both `MPISender` and `MPIReceiver` obtain the MPI communication channel reading an `MPIToken` from the event,
identified by the `upstream` parmeter.
