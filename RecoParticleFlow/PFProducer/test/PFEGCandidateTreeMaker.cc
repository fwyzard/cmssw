...
void PFEGCandidateTreeMaker::processEGCandidateFillTree(const edm::Event& e,
  ...
  for (auto clus = sc.clustersBegin(); clus != clusend; ++clus) {
    ...
    clusterDPhiToSeed.get()[iclus] = TVector2::Phi_mpi_pi(pclus->phi() - theseed->phi());
    clusterDEtaToSeed.get()[iclus] = pclus->eta() - theseed->eta();
    clusterDPhiToCentroid.get()[iclus] = TVector2::Phi_mpi_pi(pclus->phi() - sc.phi());
    ...
    if (_dogen && genmatch.isNonnull()) {
      clusterDPhiToGen.get()[iclus] = TVector2::Phi_mpi_pi(pclus->phi() - genmatch->phi());
