...
void PFElectronAlgo::RunPFElectron(const reco::PFBlockRef& blockRef,
  ...
  bool blockHasGSF = SetLinks(blockRef, associatedToGsf, associatedToBrems, associatedToEcal, active, primaryVertex);
...
bool PFElectronAlgo::SetLinks(const reco::PFBlockRef& blockRef,
  ...
  for (unsigned int iEle = 0; iEle < elements.size(); iEle++) {
    ...
    switch (type) {
      ...
      case PFBlockElement::GSF:
        ...
        block.associatedElements(iEle, linkData, kfElems, reco::PFBlockElement::TRACK, reco::PFBlock::LINKTEST_ALL);
        thisIsAMuon = !kfElems.empty() ? PFMuonAlgo::isMuon(elements[kfElems.begin()->second]) : false;
        // Otherwise store index
        if (!thisIsAMuon && active[iEle]) {
  ...
  if (IsThereAGSFTrack) {
    ...
    for (unsigned int iEle = 0; iEle < trackIs.size(); iEle++) {
      ...
      block.associatedElements(
          trackIs[iEle], linkData, gsfElems, reco::PFBlockElement::GSF, reco::PFBlock::LINKTEST_ALL);
      if (gsfElems.empty()) {
        // This means that the considered kf is *not* associated
        // to any gsf track
        std::multimap<double, unsigned int> ecalKfElems;
        block.associatedElements(
            trackIs[iEle], linkData, ecalKfElems, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);
        if (!ecalKfElems.empty()) {
          ...
          if (localactive[ecalKf_index] == true) {
            ...
            for (unsigned int iGsf = 0; iGsf < gsfIs.size(); iGsf++) {
              ...
              if (GsfEl->trackType(reco::PFBlockElement::T_FROM_GAMMACONV))
                continue;

              std::multimap<double, unsigned int> ecalGsfElems;
              block.associatedElements(
                  gsfIs[iGsf], linkData, ecalGsfElems, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);
            ...
            if (isGsfLinked == false) {
              ...
              for (Vertex::trackRef_iterator trackIt = primaryVertex.tracks_begin();
                   trackIt != primaryVertex.tracks_end();
                   ++trackIt) {
              ...
              if (isGoodTrack && nexhits == 0 && trackIsFromPrimaryVertex) {
    ...
    for (unsigned int iEle = 0; iEle < gsfIs.size(); iEle++) {
      ...
      if (DebugSetLinksDetailed)
        cout << " Gsf Index " << gsfIs[iEle] << endl;
      ...
      std::multimap<double, unsigned int> kfElems;
      block.associatedElements(
          gsfIs[iEle], linkData, kfElems, reco::PFBlockElement::TRACK, reco::PFBlock::LINKTEST_ALL);
      std::multimap<double, unsigned int> ecalKfElems;
      if (!kfElems.empty()) {
        // 19 Mar 2010 now a loop is needed because > 1 KF track could
        // be associated to the same GSF track

        for (std::multimap<double, unsigned int>::iterator itkf = kfElems.begin(); itkf != kfElems.end(); ++itkf) {
          ...
          if (localactive[itkf->second] == true) {
            ...
            block.associatedElements(
                KfGsf_index, linkData, ecalKfElems, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);
          } else {
      ...
      std::multimap<double, unsigned int> ecalGsfElems;
      block.associatedElements(
          gsfIs[iEle], linkData, ecalGsfElems, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);
      ...
      if (!ecalGsfElems.empty()) {
        if (localactive[(ecalGsfElems.begin()->second)] == true) {
          ...
          if (compatibleEPout) {
            ...
            ecalGsf_dist = block.dist(gsfIs[iEle], ClosestEcalGsf_index, linkData, reco::PFBlock::LINKTEST_ALL);

            // Check that this cluster is not closer to another primary Gsf track

            std::multimap<double, unsigned int> ecalOtherGsfElems;
            block.associatedElements(ClosestEcalGsf_index,
                                     linkData,
                                     ecalOtherGsfElems,
      ...
      // if any cluster is found with the gsf-ecal link, try with kf-ecal
        ...
        if (localactive[(ecalKfElems.begin()->second)] == true) {
          ...
          std::multimap<double, unsigned int> ecalOtherGsfElems;
          block.associatedElements(
              ClosestEcalGsf_index, linkData, ecalOtherGsfElems, reco::PFBlockElement::GSF, reco::PFBlock::LINKTEST_ALL);
          if (!ecalOtherGsfElems.empty()) {
            const reco::PFBlockElementGsfTrack* gsfCheck =
                dynamic_cast<const reco::PFBlockElementGsfTrack*>((&elements[ecalOtherGsfElems.begin()->second]));
      ...
      if (DebugSetLinksDetailed)
        cout << " Closest Ecal to the Gsf/Kf: index " << ClosestEcalGsf_index << " dist " << ecalGsf_dist << endl;

      //  Find the brems associated to this Gsf
      std::multimap<double, unsigned int> bremElems;
      block.associatedElements(
          gsfIs[iEle], linkData, bremElems, reco::PFBlockElement::BREM, reco::PFBlock::LINKTEST_ALL);
      ...
      for (std::multimap<double, unsigned int>::iterator ieb = bremElems.begin(); ieb != bremElems.end(); ++ieb) {
        unsigned int brem_index = ieb->second;
        if (localactive[brem_index] == false)
          continue;
        ...
        block.associatedElements(
            brem_index, linkData, ecalBremsElems, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);

        for (std::multimap<double, unsigned int>::iterator ie = ecalBremsElems.begin(); ie != ecalBremsElems.end();
             ie++) {
          unsigned int ecalBrem_index = ie->second;
          if (localactive[ecalBrem_index] == false)
            continue;

          //to be changed, using the distance
          float ecalBrem_dist = block.dist(brem_index, ecalBrem_index, linkData, reco::PFBlock::LINKTEST_ALL);

          if (ecalBrem_index == ClosestEcalGsf_index && (ecalBrem_dist + 0.0012) > ecalGsf_dist)
            continue;

          // Find the closest brem
          std::multimap<double, unsigned int> sortedBremElems;
          block.associatedElements(
              ecalBrem_index, linkData, sortedBremElems, reco::PFBlockElement::BREM, reco::PFBlock::LINKTEST_ALL);
          ...
          if (isGoodBrem) {
            ...
            std::multimap<double, unsigned int> ecalOtherGsfElems;
            block.associatedElements(
                ecalBrem_index, linkData, ecalOtherGsfElems, reco::PFBlockElement::GSF, reco::PFBlock::LINKTEST_ALL);
            if (!ecalOtherGsfElems.empty()) {
              const reco::PFBlockElementGsfTrack* gsfCheck =
                  dynamic_cast<const reco::PFBlockElementGsfTrack*>((&elements[ecalOtherGsfElems.begin()->second]));
              if (ecalOtherGsfElems.begin()->second != gsfIs[iEle] &&
            ...
            reco::PFClusterRef clusterRef = elements[ecalBrem_index].clusterRef();
            ...
            if (sortedBremEcal_deta < 0.015) {
              ...
              if (DebugSetLinksDetailed)
                cout << " brem Index " << sortedBrem_index << " associated cluster " << ecalBrem_index << " BremTrajP "
                     << BremTrajP << endl;
              ...
              for (unsigned int ii = 0; ii < keyBremIndex.size(); ii++) {
                if (sortedBrem_index == keyBremIndex[ii])
                  alreadyfound = true;
              }
      ...
      if (ClosestEcalGsf_index < CutIndex) {
        ...
        for (std::multimap<double, unsigned int>::iterator ii = ecalGsfElems.begin(); ii != ecalGsfElems.end(); ii++) {
          if (localactive[ii->second]) {
            // Check that this cluster is not closer to another Gsf Track
            std::multimap<double, unsigned int> ecalOtherGsfElems;
            block.associatedElements(
                ii->second, linkData, ecalOtherGsfElems, reco::PFBlockElement::GSF, reco::PFBlock::LINKTEST_ALL);
            if (!ecalOtherGsfElems.empty()) {
              if (ecalOtherGsfElems.begin()->second != gsfIs[iEle])
                continue;
            }
      ...
      if (GsfElemIndex.empty()) {
        if (latestBrem_index < CutIndex) {
          ...
          if (ckey == 1) {
            unsigned int temp_cal = cleanedEcalBremElems.find(latestBrem_index)->second;
            GsfElemIndex.push_back(temp_cal);
            if (DebugSetLinksDetailed)
              cout << "******************** Gsf Cluster From Brem " << temp_cal << " Latest Brem index "
                   << latestBrem_index << " ************************* " << endl;
          } else {
            ...
            for (it = ret.first; it != ret.second; ++it) {
              ...
              if (DebugSetLinksDetailed)
                cout << "******************** Gsf Cluster From Brem " << (*it).second << " Latest Brem index "
                     << latestBrem_index << " ************************* " << endl;
            }
          }
          // erase the brem.
      ...
      for (unsigned int iConv = 0; iConv < gsfIs.size(); iConv++) {
        if (iConv != iEle) {
          ...
          if (gsfConv->trackType(reco::PFBlockElement::T_FROM_GAMMACONV)) {
            ...
            float conv_dist = block.dist(gsfIs[iConv], gsfIs[iEle], linkData, reco::PFBlock::LINKTEST_ALL);
            if (conv_dist > 0.) {
              // find the closest ecal cluster associated to conversions

              std::multimap<double, unsigned int> ecalConvElems;
              block.associatedElements(
                  gsfIs[iConv], linkData, ecalConvElems, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);
              if (!ecalConvElems.empty()) {
                ...
                if (localactive[(ecalConvElems.begin()->second)] == true) {
                  ...
                  block.associatedElements(ecalConvElems.begin()->second,
                                           linkData,
      ...
      for (unsigned int i = 0; i < keyBremIndex.size(); i++) {
        ...
        if (ckey == 1) {
          unsigned int temp_cal = cleanedEcalBremElems.find(ikey)->second;
      ...
      if (!kfElems.empty()) {
        for (std::multimap<double, unsigned int>::iterator itkf = kfElems.begin(); itkf != kfElems.end(); ++itkf) {
          ...
          if (!isPrim) {
            ...
            std::multimap<double, unsigned int> ecalConvElems;
            block.associatedElements(
                itkf->second, linkData, ecalConvElems, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);
            if (!ecalConvElems.empty()) {
              ...
              block.associatedElements(
                  itkf->second, linkData, hcalConvElems, reco::PFBlockElement::HCAL, reco::PFBlock::LINKTEST_ALL);
              ...
              if (localactive[(ecalConvElems.begin()->second)] == false) {
                if (isHoE || isPoHE) {
                  if (DebugSetLinksDetailed)
                    cout << "PFElectronAlgo:: LOCKED ECAL REJECTED TRACK FOR H/E or P/(H+E) "
                         << " H/H+E " << enehcalclust / (enehcalclust + eneclust) << " H/E " << enehcalclust / eneclust
                         << " P/(H+E) " << secpin / (enehcalclust + eneclust) << " HCAL ENE " << enehcalclust
                         << " ECAL ENE " << eneclust << " secPIN " << secpin << " Algo Track " << trkRef->algo()
                         << endl;
                  continue;
                }
                ...
                for (unsigned int iecal = 0; iecal < EcalIndex.size(); iecal++) {
                  ...
                  if (EcalIndex[iecal] == ecalConvElems.begin()->second) {
                    if (DebugSetLinksDetailed)
                      cout << " PFElectronAlgo:: Conv Brem Recovery locked cluster and I will lock also the KF track "
                           << endl;
                ...
                if (isHoHE) {
                  if (DebugSetLinksDetailed)
                    cout << "PFElectronAlgo:: FREE ECAL REJECTED TRACK FOR H/H+E "
                         << " H/H+E " << (enehcalclust / (enehcalclust + eneclust)) << " H/E "
                         << enehcalclust / eneclust << " P/(H+E) " << secpin / (enehcalclust + eneclust) << " HCAL ENE "
                         << enehcalclust << " ECAL ENE " << eneclust << " secPIN " << secpin << " Algo Track "
                         << trkRef->algo() << endl;
                  continue;
                }
                ...
                if (!ecalOtherKFPrimElems.empty()) {
                  ...
                  for (std::multimap<double, unsigned int>::iterator itclos = kfElems.begin(); itclos != kfElems.end();
                       ++itclos) {
                  ...
                  if (isFromGSF) {
                    ...
                    float totenergy = 0.;
                    for (unsigned int ikeyecal = 0; ikeyecal < EcalIndex.size(); ikeyecal++) {
                    ...
                    if (Epin > 3) {
                      ...
                      if (res_before < res_after) {
                        if (DebugSetLinksDetailed)
                          cout << "PFElectronAlgo::REJECTED_RES totenergy " << totenergy << " Pin_gsf " << Pin_gsf
                               << " cluster to secondary " << eneclust << " Res before " << res_before << " res_after "
                               << res_after << endl;
                        continue;
                      }
                    }
      ...
      if (!EcalIndex.empty() && useEGammaSupercluster_) {
        ...
        for (unsigned int iEcal = 0; iEcal < ecalIs.size(); iEcal++) {
          ...
          if (foundcluster == false) {
            ...
            if (dphi_clust < -M_PI)
              ...
              dphi_clust = dphi_clust - 2. * M_PI;
            double DR = sqrt(deta_clust * deta_clust + dphi_clust * dphi_clust);
            ...
            if (fabs(deta_clust) > 0.05 && DR < coneEcalIsoForEgammaSC_) {
              ...
              double ps1, ps2;
              ps1 = ps2 = 0.;
              double EE_calib = thePFEnergyCalibration_->energyEm(
                  *(clustExt->clusterRef()), ps1Ene, ps2Ene, ps1, ps2, applyCrackCorrections_);
          ...
          if (localactive[(trackIs[iTrack])] == true) {
            ...
            if (trkref.isNonnull()) {
              ...
              if (dphi_trk < -M_PI)
                ...
                dphi_trk = dphi_trk - 2. * M_PI;
              double DR = sqrt(deta_trk * deta_trk + dphi_trk * dphi_trk);
        ...
        if (DebugSetLinksDetailed) {
          if (fabs(EtaFC < 1.478) && isBarrelIsolated == false) {
            cout << "**** PFElectronAlgo:: SUPERCLUSTER FOUND BUT FAILS ISOLATION:BARREL *** "
                 << " sumEtEcalInTheCone " << sumEtEcalInTheCone << " sumNTracksInTheCone " << sumNTracksInTheCone
        ...
        if (isBarrelIsolated || isEndcapIsolated) {
          ...
          for (unsigned int ikeyecal = 0; ikeyecal < EcalIndex.size(); ikeyecal++) {
            ...
            block.associatedElements(EcalIndex[ikeyecal],
                                     linkData,
            ...
            if (!ecalFromSuperClusterElems.empty()) {
              for (std::multimap<double, unsigned int>::iterator itsc = ecalFromSuperClusterElems.begin();
                   itsc != ecalFromSuperClusterElems.end();
                   ++itsc) {
                ...
                std::multimap<double, unsigned int> ecalOtherKFPrimElems;
                block.associatedElements(itsc->second,
                                         linkData,
                                         ecalOtherKFPrimElems,
                                         reco::PFBlockElement::TRACK,
                                         reco::PFBlock::LINKTEST_ALL);
                    ...
                    if (res_after_gsf < res_before_gsf && res_after_kf < res_before_kf) {
                      ...
                      if (DebugSetLinksDetailed)
                        cout << "**** PFElectronAlgo:: SUPERCLUSTER FOUND AND FAILS ALSO RES_EPIN"
                             << " tot energy " << totenergy << " Pin_gsf " << Pin_gsf << " Pin_kf " << Pin_kf
                             << " cluster from SC to ADD " << ene_clustToAdd << " Res before GSF " << res_before_gsf
                             << " res_after_gsf " << res_after_gsf << " Res before KF " << res_before_kf
      ...
      for (unsigned int ikeyecal = 0; ikeyecal < EcalIndex.size(); ikeyecal++) {
        ...
        block.associatedElements(
            EcalIndex[ikeyecal], linkData, PS1Elems, reco::PFBlockElement::PS1, reco::PFBlock::LINKTEST_ALL);
        for (std::multimap<double, unsigned int>::iterator it = PS1Elems.begin(); it != PS1Elems.end(); it++) {
          ...
          if (localactive[index] == true) {
            ...
            block.associatedElements(
                index, linkData, sortedECAL, reco::PFBlockElement::ECAL, reco::PFBlock::LINKTEST_ALL);
            unsigned jEcal = sortedECAL.begin()->second;
            if (jEcal != EcalIndex[ikeyecal])
              continue;
        ...
        block.associatedElements(
            EcalIndex[ikeyecal], linkData, PS2Elems, reco::PFBlockElement::PS2, reco::PFBlock::LINKTEST_ALL);
        ...
        if (ikeyecal == 0) {
          ...
          block.associatedElements(
              gsfIs[iEle], linkData, hcalGsfElems, reco::PFBlockElement::HCAL, reco::PFBlock::LINKTEST_ALL);
          for (std::multimap<double, unsigned int>::iterator it = hcalGsfElems.begin(); it != hcalGsfElems.end();
               it++) {
            unsigned int index = it->second;
            //  if(localactive[index] == true) {
          ...
          if (hcalGsfElems.empty() && ClosestEcalWithKf == true) {
            ...
            block.associatedElements(
                KfGsf_index, linkData, hcalKfElems, reco::PFBlockElement::HCAL, reco::PFBlock::LINKTEST_ALL);
            for (std::multimap<double, unsigned int>::iterator it = hcalKfElems.begin(); it != hcalKfElems.end();
                 it++) {
              unsigned int index = it->second;
              if (localactive[index] == true) {
                ...
                block.associatedElements(
                    index, linkData, sortedKf, reco::PFBlockElement::TRACK, reco::PFBlock::LINKTEST_ALL);
          ...
          block.associatedElements(
              EcalIndex[ikeyecal], linkData, kfEtraElems, reco::PFBlockElement::TRACK, reco::PFBlock::LINKTEST_ALL);
          if (!kfEtraElems.empty()) {
            for (std::multimap<double, unsigned int>::iterator it = kfEtraElems.begin(); it != kfEtraElems.end();
                 it++) {
              unsigned int index = it->second;
  ...
  if (DebugSetLinksSummary) {
    if (IsThereAGoodGSFTrack) {
      if (DebugSetLinksSummary)
        cout << " -- The Link Summary --" << endl;
      for (map<unsigned int, vector<unsigned int> >::iterator it = associatedToGsf_.begin();
           it != associatedToGsf_.end();
           it++) {
        if (DebugSetLinksSummary)
          cout << " AssoGsf " << it->first << endl;
      ...
      for (map<unsigned int, vector<unsigned int> >::iterator it = associatedToBrems_.begin();
           it != associatedToBrems_.end();
           it++) {
        if (DebugSetLinksSummary)
          cout << " AssoBrem " << it->first << endl;
        vector<unsigned int> eleasso = it->second;
        for (unsigned int i = 0; i < eleasso.size(); i++) {
          PFBlockElement::Type type = elements[eleasso[i]].type();
      ...
      for (map<unsigned int, vector<unsigned int> >::iterator it = associatedToEcal_.begin();
           it != associatedToEcal_.end();
           it++) {
        if (DebugSetLinksSummary)
          cout << " AssoECAL " << it->first << endl;
        vector<unsigned int> eleasso = it->second;
        for (unsigned int i = 0; i < eleasso.size(); i++) {
          PFBlockElement::Type type = elements[eleasso[i]].type();
...
void PFElectronAlgo::SetIDOutputs(const reco::PFBlockRef& blockRef,
  ...
  for (map<unsigned int, vector<unsigned int> >::iterator igsf = associatedToGsf_.begin();
    ...
    if (RefGSF.isNonnull()) {
      float m_el = 0.00051;
      Ein_gsf = sqrt(RefGSF->pMode() * RefGSF->pMode() + m_el * m_el);
    ...
    if (DebugIDOutputs)
      cout << " setIdOutput! GSF Track: Ein " << Ein_gsf << " eta,phi " << Etaout_gsf << ", "
    ...
    for (unsigned int ielegsf = 0; ielegsf < assogsf_index.size(); ielegsf++) {
      ...
      if (assoele_type == reco::PFBlockElement::ECAL) {
        ...
        if (FirstEcalGsf) {
          ...
          Ene_ecalgsf =
              thePFEnergyCalibration_->energyEm(*clusterRef, ps1Ene, ps2Ene, ps1, ps2, applyCrackCorrections_);
          ...
          if (DebugIDOutputs)
            cout << " setIdOutput! GSF ECAL Cluster E " << Ene_ecalgsf << " eta,phi " << clusterRef->position().eta()
                 << ", " << clusterRef->position().phi() << endl;
          ...
          reco::PFClusterRef clusterRef = elements[(assogsf_index[ielegsf])].clusterRef();
          float TempClus_energy =
              thePFEnergyCalibration_->energyEm(*clusterRef, ps1Ene, ps2Ene, applyCrackCorrections_);
          ...
          if (DebugIDOutputs)
            cout << " setIdOutput! Extra ECAL Cluster E " << TempClus_energy << " Tot " << Ene_extraecalgsf << endl;
        }
      }  // end type Ecal

      if (assoele_type == reco::PFBlockElement::BREM) {
        unsigned int brem_index = assogsf_index[ielegsf];
        const reco::PFBlockElementBrem* BremEl = dynamic_cast<const reco::PFBlockElementBrem*>((&elements[brem_index]));
        int TrajPos = (BremEl->indTrajPoint()) - 2;
        //if (TrajPos <= 3) EarlyBrem = true;
        if (TrajPos < FirstBrem)
          FirstBrem = TrajPos;
        ...
        for (unsigned int ibrem = 0; ibrem < assobrem_index.size(); ibrem++) {
          if (elements[(assobrem_index[ibrem])].type() == reco::PFBlockElement::ECAL) {
            ...
            if (assobrem_index[ibrem] != ecalGsf_index) {
              ...
              float BremClus_energy = thePFEnergyCalibration_->energyEm(
                  *clusterRef, ps1EneFromBrem, ps2EneFromBrem, applyCrackCorrections_);
              ...
              NumBrem++;
              if (DebugIDOutputs)
                cout << " setIdOutput::BREM Cluster " << BremClus_energy << " eta,phi " << clusterRef->position().eta()
                     << ", " << clusterRef->position().phi() << endl;
            } else {
              LateBrem = true;
            }
    ...
    if (Ene_ecalgsf > 0.) {
      ...
      if (RefGSF.isNonnull()) {
        ...
        if (DPtOverPt_gsf < -0.2)
          DPtOverPt_gsf = -0.2;
        if (DPtOverPt_gsf > 1.)
          DPtOverPt_gsf = 1.;

        if (dPtOverPt_gsf > 0.3)
          dPtOverPt_gsf = 0.3;
        ...
        if (EtotBremPinPoutMode < 0.)
          EtotBremPinPoutMode = 0.01;
        if (EtotBremPinPoutMode > 5.)
          EtotBremPinPoutMode = 5.;

        if (DEtaGsfEcalClust > 0.1)
          DEtaGsfEcalClust = 0.1;
        ...
        if (mvaValue > mvaEleCut_) {
            ...
            for (unsigned int itrk = 0; itrk < assoecalgsf_index.size(); itrk++) {
              ...
              if (typeassoecal == reco::PFBlockElement::TRACK) {
                ...
                if (goodTrack && nexhits == 0 && trackIsFromPrimaryVertex) {
                  ...
                  if (DebugIDOutputs)
                    cout << "  p_trk " << p_trk << " nexhits " << nexhits << endl;

                  SumExtraKfP += p_trk;
                  iextratrack++;
                  // Check if these extra tracks are HCAL linked
                  std::multimap<double, unsigned int> hcalKfElems;
                  block.associatedElements(assoecalgsf_index[itrk],
                                           linkData,
                                           hcalKfElems,
          ...
          if (iextratrack > 0) {
            ...
            if (iextratrack > 3 || Ene_hcalgsf > 10 || (SumExtraKfP / Ene_ecalgsf) > 1. ||
              ...
              if (DebugIDOutputs)
                cout << " *****This electron candidate is discarded: Non isolated  # tracks " << iextratrack
                     << " HOverHE " << HOverHE << " SumExtraKfP/Ene_ecalgsf " << SumExtraKfP / Ene_ecalgsf
                     << " SumExtraKfP " << SumExtraKfP << " Ene_ecalgsf " << Ene_ecalgsf << " ETtotal " << ETtotal
                     << " Ene_hcalgsf/Ene_ecalgsf " << Ene_hcalgsf / Ene_ecalgsf << endl;
            ...
            if ((fabs(1. - EtotPinMode) < 0.2 && (fabs(Eta_gsf) < 1.0 || fabs(Eta_gsf) > 2.0)) ||
              ...
              if (fabs(1. - EGsfPoutMode) < 0.5 && (itrackHcalLinked == iextratrack) && kf_index < 100000) {
                BDToutput_[cgsf] = mvaValue;
                lockExtraKf_[cgsf] = false;
                if (DebugIDOutputs)
                  cout << " *****This electron is reactivated  # tracks " << iextratrack << " #tracks hcal linked "
                       << itrackHcalLinked << " SumExtraKfP/Ene_ecalgsf " << SumExtraKfP / Ene_ecalgsf
                       << " EtotPinMode " << EtotPinMode << " EGsfPoutMode " << EGsfPoutMode << " eta gsf "
                       << fabs(Eta_gsf) << " kf index " << kf_index << endl;
              }
            }
          }
          // This is a pion:
        ...
        if (DebugIDOutputs) {
          ...
          cout << " Pt_gsf " << Pt_gsf << " Pin " << Ein_gsf << " Pout " << Eout_gsf << " Eta_gsf " << Eta_gsf << endl;
          cout << " < PureTracking > " << endl;
          cout << " dPtOverPt_gsf " << dPtOverPt_gsf << " DPtOverPt_gsf " << DPtOverPt_gsf << " chi2_gsf " << chi2_gsf
               << " nhit_gsf " << nhit_gsf << " DPtOverPt_kf " << DPtOverPt_kf << " chi2_kf " << chi2_kf << " nhit_kf "
               << nhit_kf << endl;
          cout << " < track-ecal-hcal-ps " << endl;
          cout << " EtotPinMode " << EtotPinMode << " EGsfPoutMode " << EGsfPoutMode << " EtotBremPinPoutMode "
               << EtotBremPinPoutMode << " DEtaGsfEcalClust " << DEtaGsfEcalClust << " SigmaEtaEta " << SigmaEtaEta
               << " HOverHE " << HOverHE << " Hcal energy " << Ene_hcalgsf << " HOverPin " << HOverPin << " lateBrem "
               << lateBrem << " firstBrem " << firstBrem << endl;
          cout << " !!!!!!!!!!!!!!!! the BDT output !!!!!!!!!!!!!!!!!: direct " << mvaValue << " corrected "
...
void PFElectronAlgo::SetCandidates(const reco::PFBlockRef& blockRef,
  ...
  for (map<unsigned int, vector<unsigned int> >::iterator igsf = associatedToGsf_.begin();
    ...
    if (RefGSF.isNonnull()) {
      ...
      float ENE = sqrt(RefGSF->pMode() * RefGSF->pMode() + m_el * m_el);

      if (DebugIDCandidates)
        cout << "SetCandidates:: GsfTrackRef: Ene " << ENE << " charge " << charge << " nhits " << nhit_gsf << endl;

      momentum_gsf.SetE(ENE);
      dpt_gsf = RefGSF->ptModeError() * (RefGSF->pMode() / RefGSF->ptMode());

      momentum_mean.SetPx(RefGSF->px());
      momentum_mean.SetPy(RefGSF->py());
      momentum_mean.SetPz(RefGSF->pz());
      float ENEm = sqrt(RefGSF->p() * RefGSF->p() + m_el * m_el);
	...
	*/
        ...
        if (DebugIDCandidates)
          cout << "SetCandidates:: EcalCluster: EneNoCalib " << clust->clusterRef()->energy() << " eta,phi " << ceta
               << "," << cphi << " Calib " << EE << " dE " << dE << endl;
        ...
        clusterMomentum.SetPxPyPzE(EE * direction.x(), EE * direction.y(), EE * direction.z(), EE);
        ...
        std::map<unsigned int, std::vector<reco::PFCandidate> >::iterator itcheck = electronConstituents_.find(cgsf);
        if (itcheck == electronConstituents_.end()) {
          // beurk
          std::vector<reco::PFCandidate> tmpVec;
          tmpVec.push_back(cluster_Candidate);
          electronConstituents_.insert(std::pair<unsigned int, std::vector<reco::PFCandidate> >(cgsf, tmpVec));
        } else {
        ...
        for (unsigned int ibrem = 0; ibrem < assobrem_index.size(); ibrem++) {
          if (elements[(assobrem_index[ibrem])].type() == reco::PFBlockElement::ECAL) {
            ...
            if (assobrem_index[ibrem] != ecalGsf_index) {
              ...
              double ps1 = 0;
              double ps2 = 0;
              float EE = thePFEnergyCalibration_->energyEm(
                  *clusterRef, ps1EneFromBrem, ps2EneFromBrem, ps1, ps2, applyCrackCorrections_);
              ...
              photonMomentum.SetPxPyPzE(EE * direction.x(), EE * direction.y(), EE * direction.z(), EE);
    ...
    if (has_gsf) {
      ...
      if (DebugIDCandidates)
        cout << "PFEelectronAlgo:: absEta " << absEta << " theta " << emTheta << " EneRaw " << Eene << " Err " << dene;
      ...
      if (has_kf && unCorrEene > 0.) {
        ...
        std::multimap<double, unsigned int> bremElems;
        block.associatedElements(
            gsf_index, linkData, bremElems, reco::PFBlockElement::BREM, reco::PFBlock::LINKTEST_ALL);
        ...
        if (DebugIDCandidates)
          cout << "PFElectronAlgo:: charge determination "
               << " charge GSF " << chargeGsf << " charge KF " << chargeKf << " charge SC " << chargeSC
      ...
      if ((nhit_gsf < 8) && (has_kf)) {
        ...
        newmomentum.SetPxPyPzE(scale * momentum.Px(), scale * momentum.Py(), scale * momentum.Pz(), Fe);
        if (DebugIDCandidates)
      ...
      if ((nhit_gsf > 7) || (has_kf == false)) {
        ...
        if ((de_gs < -0.1) && (de_me < -0.1) && (de_kf < 0.) && (momentum.E() / dpt_gsf) > 5. &&
            momentum_gsf.pt() < 30.) {
          Fe = momentum.E();
        }
        float scale = Fe / momentum.E();
      ...
      if (newmomentum.pt() > 0.5) {
        ...
        if (itextra != electronExtra_.end()) {
          ...
          if (RawEene > 0.)
            std::cout << " There is a big problem with the electron extra, PFElectronAlgo should crash soon " << RawEene
                      << std::endl;
        }
        ...
        if (useEGElectrons_) {
          auto itcheck = find_if(theGsfElectrons_->begin(), theGsfElectrons_->end(), [&RefGSF](const auto& ele) {
            ...
            if (BDToutput_[cgsf] >= -1.) {
              ...
              if (DebugIDCandidates) {
                if (BDToutput_[cgsf] < -0.1) {
                  ...
                  cout << " Attention By pass the mva " << BDToutput_[cgsf] << " SuperClusterEnergy " << esceg
                       << " PF Energy " << Eene << endl;

                  cout << " hoe " << itcheck->hcalOverEcal() << " tkiso04 " << itcheck->dr04TkSumPt() << " ecaliso04 "
                       << itcheck->dr04EcalRecHitSumEt() << " hcaliso04 " << itcheck->dr04HcalTowerSumEt()
                       << " tkiso03 " << itcheck->dr03TkSumPt() << " ecaliso03 " << itcheck->dr03EcalRecHitSumEt()
...
void PFElectronAlgo::SetActive(const reco::PFBlockRef& blockRef,
  ...
  for (map<unsigned int, vector<unsigned int> >::iterator igsf = associatedToGsf_.begin();
    ...
    if (BDToutput_[cgsf] < mvaEleCut_ && bypassmva == false)
      continue;
...
bool PFElectronAlgo::isPrimaryTrack(const reco::PFBlockElementTrack& KfEl, const reco::PFBlockElementGsfTrack& GsfEl) {
