...
PFMuonAlgo* PFAlgo::getPFMuonAlgo() { return pfmu_; }
...
void PFAlgo::setPFEleParameters(double mvaEleCut,
  ...
  pfele_ = std::make_unique<PFElectronAlgo>(mvaEleCut_,
                                            mvaWeightFileEleID_,
...
void PFAlgo::setPFPhotonParameters(bool usePFPhotons,
...
void PFAlgo::setPFPhotonRegWeights(const GBRForest* LCorrForestEB,
  ...
  pfpho_->setGBRForest(
      LCorrForestEB, LCorrForestEE, GCorrForestBarrel, GCorrForestEndcapHr9, GCorrForestEndcapLr9, PFEcalResolution);
}
...
void PFAlgo::processBlock(const reco::PFBlockRef& blockref,
  ...
  if (usePFElectrons_) {
    ...
    pfElectronExtra_.insert(
        pfElectronExtra_.end(), pfele_->getElectronExtra().begin(), pfele_->getElectronExtra().end());
  }
  if (/* --- */ usePFPhotons_ /* --- */) {
    if (debug_)
  ...
  for (unsigned iEle = 0; iEle < elements.size(); iEle++) {
    ...
    if (debug_) {
      cout << "PFAlgo:processBlock "
           << " " << trackIs.size() << " " << ecalIs.size() << " " << hcalIs.size() << " " << hoIs.size() << endl;
    }
    ...
    if (hcalElems.empty()) {
      if (debug_)
        std::cout << "Now deals with tracks linked to no HCAL clusters. Was HCal active? " << (!hasDeadHcal)
                  << std::endl;
      ...
      if (!thisIsAMuon && elements[iTrack].trackRef()->ptError() > ptError_) {
        ...
        if (deficit > nSigmaTRACK_ * resol && !isPrimary && !goodTrackDeadHcal) {
          ...
          if (debug_)
            std::cout << elements[iTrack] << std::endl
                      << " deficit " << deficit << " > " << nSigmaTRACK_ << " x " << resol << " track pt "
                      << trackRef->pt() << " +- " << trackRef->ptError() << " layers valid "
                      << trackRef->hitPattern().trackerLayersWithMeasurement() << ", lost "
                      << trackRef->hitPattern().trackerLayersWithoutMeasurement(HitPattern::TRACK_HITS)
                      << ", lost outer "
                      << trackRef->hitPattern().trackerLayersWithoutMeasurement(HitPattern::MISSING_OUTER_HITS)
                      << ", lost inner "
                      << trackRef->hitPattern().trackerLayersWithoutMeasurement(HitPattern::MISSING_INNER_HITS)
                      << "(valid fraction " << trackRef->validFraction() << ")"
                      << " chi2/ndf " << trackRef->normalizedChi2() << " |dxy| "
                      << std::abs(trackRef->dxy(primaryVertex_.position())) << " +- " << trackRef->dxyError()
      ...
      if (rejectTracks_Step45_ && ecalElems.empty() && trackMomentum > 30. && Dpt > 0.5 &&
        ...
        if (debug_)
          std::cout << "A track (algo = " << trackRef->algo() << ") with momentum " << trackMomentum << " / "
                    ...
                    << elements[iTrack].trackRef()->eta() << " without any link to ECAL/HCAL and with " << nHits << " ("
                    << NLostHit << ") hits (lost hits) has been cleaned" << std::endl;
        active[iTrack] = false;
        continue;
      }
      ...
      for (auto const& ecal : ecalElems) {
        ...
        block.associatedElements(
            index, linkData, sortedTracks, reco::PFBlockElement::TRACK, reco::PFBlock::LINKTEST_ALL);
        ...
        if (connectedToEcal && calibEcal - trackMomentum >= 0.) {
          ...
          block.associatedElements(
              index, linkData, assTracks, reco::PFBlockElement::TRACK, reco::PFBlock::LINKTEST_ALL);
  ...
  if (debug_) {
    cout << endl;
    if (debug_)
      cout << endl << "---- loop remaining hcal ------- " << endl;
  }
...
void PFAlgo::setEGElectronCollection(const reco::GsfElectronCollection& egelectrons) {
  if (useEGElectrons_ && pfele_)
    pfele_->setEGElectronCollection(egelectrons);
}
...
void PFAlgo::setElectronExtraRef(const edm::OrphanHandle<reco::PFCandidateElectronExtraCollection>& extrah) {
  ...
  for (auto& cand : *pfCandidates_) {
    ...
    if (cand.particleId() == PFCandidate::e) {
      ...
      std::vector<PFCandidateElectronExtra>::const_iterator it =
          find_if(pfElectronExtra_.begin(), pfElectronExtra_.end(), myExtraEqual);
    ...
    } else  // else save the mva and the extra as well !
    {
      if (cand.trackRef().isNonnull()) {
        auto it = find_if(
            pfElectronExtra_.begin(), pfElectronExtra_.end(), [&cand](const reco::PFCandidateElectronExtra& extra) {
