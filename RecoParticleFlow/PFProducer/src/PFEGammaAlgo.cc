...
namespace {
  ...
  struct NotCloserToOther {
    ...
    bool operator()(const PFFlaggedElement& e) {
      if (!e.second || valtype != e.first->type())
        return false;
      return elementNotCloserToOther<useConv>(block, keytype, comp->index(), valtype, e.first->index(), EoPin_cut);
    }
  };
  ...
  struct LesserByDistance {
    ...
    bool operator()(const PFFlaggedElement& e1, const PFFlaggedElement& e2) {
      double dist1 = block->dist(comp->index(), e1.first->index(), links, reco::PFBlock::LINKTEST_ALL);
      double dist2 = block->dist(comp->index(), e2.first->index(), links, reco::PFBlock::LINKTEST_ALL);
  ...
  bool isROLinkedByClusterOrTrack(const PFEGammaAlgo::ProtoEGObject& RO1, const PFEGammaAlgo::ProtoEGObject& RO2) {
    ...
    for (const auto& cluster : RO1.ecalclusters) {
      for (const auto& primgsf : RO2.primaryGSFs) {
        not_closer = elementNotCloserToOther(
            blk, cluster.first->type(), cluster.first->index(), primgsf.first->type(), primgsf.first->index());
        if (not_closer) {
      ...
      for (const auto& primkf : RO2.primaryKFs) {
        not_closer = elementNotCloserToOther(
            blk, cluster.first->type(), cluster.first->index(), primkf.first->type(), primkf.first->index());
        if (not_closer) {
      ...
      for (const auto& secdkf : RO2.secondaryKFs) {
        not_closer = elementNotCloserToOther(
            blk, cluster.first->type(), cluster.first->index(), secdkf.first->type(), secdkf.first->index());
        if (not_closer) {
      ...
      for (const auto& brem : RO2.brems) {
        not_closer = elementNotCloserToOther(
            blk, cluster.first->type(), cluster.first->index(), brem.first->type(), brem.first->index());
        if (not_closer) {
    ...
    for (const auto& primgsf : RO1.primaryGSFs) {
      for (const auto& secdkf : RO2.secondaryKFs) {
        not_closer = elementNotCloserToOther(
            blk, primgsf.first->type(), primgsf.first->index(), secdkf.first->type(), secdkf.first->index());
        if (not_closer) {
    ...
    for (const auto& primkf : RO1.primaryKFs) {
      for (const auto& secdkf : RO2.secondaryKFs) {
        not_closer = elementNotCloserToOther(
            blk, primkf.first->type(), primkf.first->index(), secdkf.first->type(), secdkf.first->index());
        if (not_closer) {
    ...
    for (const auto& secdkf1 : RO1.secondaryKFs) {
      for (const auto& secdkf2 : RO2.secondaryKFs) {
        not_closer = elementNotCloserToOther<true>(
            blk, secdkf1.first->type(), secdkf1.first->index(), secdkf2.first->type(), secdkf2.first->index());
        if (not_closer) {
  ...
  std::vector<const ClusterElement*> getSCAssociatedECALsSafe(const reco::SuperClusterRef& scref,
    ...
    for (; sccl != scend; ++sccl) {
      ...
      for (pfc = ecals.begin(); pfc != pfcend; ++pfc) {
        const ClusterElement* pfcel = docast(const ClusterElement*, pfc->first);
  ...
  bool addPFClusterToROSafe(const ClusterElement* cl, PFEGammaAlgo::ProtoEGObject& RO) {
    if (RO.ecalclusters.empty()) {
      ...
      const PFLayer::Layer blayer = RO.ecalclusters.back().first->clusterRef()->layer();
      if (clayer == blayer) {
        RO.ecalclusters.emplace_back(cl, true);
        return true;
      }
  ...
  void setROElectronCluster(PFEGammaAlgo::ProtoEGObject& RO) {
    ...
    for (const auto& cluster : RO.ecalclusters) {
      for (const auto& gsf : RO.primaryGSFs) {
        const bool hasclu = elementNotCloserToOther(
            parent, gsf.first->type(), gsf.first->index(), cluster.first->type(), cluster.first->index());
        const float deta =
            std::abs(cluster.first->clusterRef()->positionREP().eta() - gsf.first->positionAtECALEntrance().eta());
        const float dphi = std::abs(TVector2::Phi_mpi_pi(cluster.first->clusterRef()->positionREP().phi() -
      ...
      for (const auto& kf : RO.primaryKFs) {
        const bool hasclu = elementNotCloserToOther(
            parent, kf.first->type(), kf.first->index(), cluster.first->type(), cluster.first->index());
        const float dist =
            parent->dist(cluster.first->index(), kf.first->index(), parent->linkData(), reco::PFBlock::LINKTEST_ALL);
      ...
      for (const auto& brem : RO.brems) {
        const bool hasclu = elementNotCloserToOther(
            parent, brem.first->type(), brem.first->index(), cluster.first->type(), cluster.first->index());
        if (hasclu) {
          ++nBremClusters;
          if (!firstBrem || (firstBrem->indTrajPoint() - 2 > brem.first->indTrajPoint() - 2)) {
            firstBrem = brem.first;
          }
          if (!lastBrem || (lastBrem->indTrajPoint() - 2 < brem.first->indTrajPoint() - 2)) {
            lastBrem = brem.first;
...
PFEGammaAlgo::PFEGammaAlgo(const PFEGammaAlgo::PFEGConfigInfo& cfg) : cfg_(cfg) {}
...
void PFEGammaAlgo::buildAndRefineEGObjects(const pfEGHelpers::HeavyObjectCache* hoc, const reco::PFBlockRef& block) {
  ...
  LOGDRESSED("PFEGammaAlgo") << "Initialized " << _refinableObjects.size() << " proto-EGamma objects" << std::endl;
  ...
  LOGDRESSED("PFEGammaAlgo") << "There are " << _refinableObjects.size() << " after the 2nd merging step." << std::endl;
  dumpCurrentRefinableObjects();
  ...
  for (auto& RO : _refinableObjects) {
    ...
    std::sort(RO.ecalclusters.begin(),
              RO.ecalclusters.end(),
              [](const PFClusterFlaggedElement& a, const PFClusterFlaggedElement& b) {
                return (a.first->clusterRef()->correctedEnergy() > b.first->clusterRef()->correctedEnergy());
              });
    setROElectronCluster(RO);
  }

  LOGDRESSED("PFEGammaAlgo") << "There are " << _refinableObjects.size() << " after the unlinking and vetos step."
                             << std::endl;
  dumpCurrentRefinableObjects();
...
void PFEGammaAlgo::initializeProtoCands(std::list<PFEGammaAlgo::ProtoEGObject>& egobjs) {
  ...
  for (auto& element : _splayedblock[PFBlockElement::SC]) {
    LOGDRESSED("PFEGammaAlgo") << "creating SC-based proto-object" << std::endl
                               << "\tSC at index: " << element.first->index() << " has type: " << element.first->type()
                               << std::endl;
  ...
  for (auto& element : _splayedblock[PFBlockElement::GSF]) {
    LOGDRESSED("PFEGammaAlgo") << "creating GSF-based proto-object" << std::endl
                               << "\tGSF at index: " << element.first->index() << " has type: " << element.first->type()
                               << std::endl;
    const PFGSFElement* elementAsGSF = docast(const PFGSFElement*, element.first);
    ...
    for (auto& brem : _splayedblock[PFBlockElement::BREM]) {
      float dist =
          _currentblock->dist(elementAsGSF->index(), brem.first->index(), _currentlinks, reco::PFBlock::LINKTEST_ALL);
      if (dist == 0.001f) {
        const PFBremElement* eAsBrem = docast(const PFBremElement*, brem.first);
...
bool PFEGammaAlgo::unwrapSuperCluster(const PFSCElement* thesc,
  ...
  for (auto ecalitr = ecalbegin; ecalitr != ecalfirstnotinsc; ++ecalitr) {
    const PFClusterElement* elemascluster = docast(const PFClusterElement*, ecalitr->first);
  ...
  for (auto hgcalitr = hgcalbegin; hgcalitr != hgcalfirstnotinsc; ++hgcalitr) {
    const PFClusterElement* elemascluster = docast(const PFClusterElement*, hgcalitr->first);
...
int PFEGammaAlgo::attachPSClusters(const ClusterElement* ecalclus, ClusterMap::mapped_type& eslist) {
  ...
  for (const auto& ps1 : _splayedblock[reco::PFBlockElement::PS1]) {
    ...
    for (auto pscl = assc_ps.first; pscl != assc_ps.second; ++pscl) {
      if (pscl->second == temp) {
        const ClusterElement* pstemp = docast(const ClusterElement*, ps1.first);
  ...
  for (const auto& ps2 : _splayedblock[reco::PFBlockElement::PS2]) {
    ...
    for (auto pscl = assc_ps.first; pscl != assc_ps.second; ++pscl) {
      if (pscl->second == temp) {
        const ClusterElement* pstemp = docast(const ClusterElement*, ps2.first);
        eslist.emplace_back(pstemp, true);
      }
    }
  }
...
void PFEGammaAlgo::dumpCurrentRefinableObjects() const {
  ...
  edm::LogVerbatim("PFEGammaAlgo")
      ...
      << "Dumping " << _refinableObjects.size() << " refinable objects for this block: " << std::endl;
...
void PFEGammaAlgo::removeOrLinkECALClustersToKFTracks() {
  ...
  for (auto& kftrack : _splayedblock[reco::PFBlockElement::TRACK]) {
    matchedGSFs.clear();
    _currentblock->associatedElements(
        kftrack.first->index(), _currentlinks, matchedGSFs, reco::PFBlockElement::GSF, reco::PFBlock::LINKTEST_ALL);
    if (matchedGSFs.empty()) {  // only run this if we aren't associated to GSF
      LesserByDistance closestTrackToECAL(_currentblock, _currentlinks, kftrack.first);
      auto ecalbegin = _splayedblock[reco::PFBlockElement::ECAL].begin();
      auto ecalend = _splayedblock[reco::PFBlockElement::ECAL].end();
      std::partial_sort(ecalbegin, ecalbegin + 1, ecalend, closestTrackToECAL);
      PFFlaggedElement& closestECAL = _splayedblock[reco::PFBlockElement::ECAL].front();
      const float dist = _currentblock->dist(
          kftrack.first->index(), closestECAL.first->index(), _currentlinks, reco::PFBlock::LINKTEST_ALL);
      bool inSC = false;
      for (auto& sc : _splayedblock[reco::PFBlockElement::SC]) {
        float dist_sc = _currentblock->dist(
            sc.first->index(), closestECAL.first->index(), _currentlinks, reco::PFBlock::LINKTEST_ALL);
        if (dist_sc != -1.0f) {
          inSC = true;
          break;
        }
      ...
      if (dist != -1.0f && closestECAL.second) {
        ...
        for (const auto& gsfflag : _splayedblock[reco::PFBlockElement::GSF]) {
          const reco::PFBlockElementGsfTrack* elemasgsf = docast(const reco::PFBlockElementGsfTrack*, gsfflag.first);
        ...
        if (!gsflinked && !inSC) {
          ...
          const reco::PFBlockElementTrack* kfEle = docast(const reco::PFBlockElementTrack*, kftrack.first);
          ...
          for (auto vtxtks = cfg_.primaryVtx->tracks_begin(); vtxtks != cfg_.primaryVtx->tracks_end(); ++vtxtks) {
  ...
  while (check_for_merge) {
    ...
    if (nomerge != mergestart) {
      ...
      for (auto roToMerge = mergestart; roToMerge != nomerge; ++roToMerge) {
        ...
        thefront.localMap.insert(thefront.localMap.end(), roToMerge->localMap.begin(), roToMerge->localMap.end());
...
void PFEGammaAlgo::linkRefinableObjectGSFTracksToKFs(ProtoEGObject& RO) {
  ...
  for (auto& gsfflagged : RO.primaryGSFs) {
    ...
    for (auto kft = KFbegin; kft != notlinked; ++kft) {
      const PFKFElement* elemaskf = docast(const PFKFElement*, kft->first);
...
void PFEGammaAlgo::linkRefinableObjectPrimaryGSFTrackToECAL(ProtoEGObject& RO) {
  ...
  for (auto& primgsf : RO.primaryGSFs) {
    NotCloserToOther<reco::PFBlockElement::GSF, reco::PFBlockElement::ECAL> gsfTracksToECALs(_currentblock,
                                                                                             primgsf.first);
    ...
    notmatched_sc = std::partition(RO.ecalclusters.begin(), notmatched_sc, eoverp_test);
    // look inside the SC for the ECAL cluster
    for (auto ecal = RO.ecalclusters.begin(); ecal != notmatched_sc; ++ecal) {
      const PFClusterElement* elemascluster = docast(const PFClusterElement*, ecal->first);
...
void PFEGammaAlgo::linkRefinableObjectPrimaryGSFTrackToHCAL(ProtoEGObject& RO) {
  ...
  for (auto& primgsf : RO.primaryGSFs) {
    NotCloserToOther<reco::PFBlockElement::GSF, reco::PFBlockElement::HCAL> gsfTracksToHCALs(_currentblock,
                                                                                             primgsf.first);
    auto notmatched = std::partition(HCALbegin, HCALend, gsfTracksToHCALs);
    for (auto hcal = HCALbegin; hcal != notmatched; ++hcal) {
      const PFClusterElement* elemascluster = docast(const PFClusterElement*, hcal->first);
      PFClusterFlaggedElement temp(elemascluster, true);
...
void PFEGammaAlgo::linkKFTrackToECAL(const KFFlaggedElement& kfflagged, ProtoEGObject& RO) {
  std::vector<PFClusterFlaggedElement>& currentECAL = RO.ecalclusters;
  auto ECALbegin = _splayedblock[reco::PFBlockElement::ECAL].begin();
  auto ECALend = _splayedblock[reco::PFBlockElement::ECAL].end();
  NotCloserToOther<reco::PFBlockElement::TRACK, reco::PFBlockElement::ECAL> kfTrackToECALs(_currentblock,
                                                                                           kfflagged.first);
  NotCloserToOther<reco::PFBlockElement::GSF, reco::PFBlockElement::ECAL> kfTrackGSFToECALs(_currentblock,
                                                                                            kfflagged.first);
...
void PFEGammaAlgo::linkRefinableObjectBremTangentsToECAL(ProtoEGObject& RO) {
  ...
  for (auto& bremflagged : RO.brems) {
    ...
    auto ECALbegin = _splayedblock[reco::PFBlockElement::ECAL].begin();
    auto ECALend = _splayedblock[reco::PFBlockElement::ECAL].end();
    NotCloserToOther<reco::PFBlockElement::BREM, reco::PFBlockElement::ECAL> BremToECALs(_currentblock,
                                                                                         bremflagged.first);
    ...
    for (auto ecal = RSCBegin; ecal != notmatched_rsc; ++ecal) {
      float deta =
          std::abs(ecal->first->clusterRef()->positionREP().eta() - bremflagged.first->positionAtECALEntrance().eta());
      if (deta < 0.015) {
        has_clusters = true;
    ...
    for (auto ecal = ECALbegin; ecal != notmatched_block; ++ecal) {
      ...
      if (deta < 0.015) {
        ...
        const PFClusterElement* elemasclus = docast(const PFClusterElement*, ecal->first);
...
void PFEGammaAlgo::linkRefinableObjectConvSecondaryKFsToSecondaryKFs(ProtoEGObject& RO) {
  ...
  for (size_t idx = 0; idx < convkfs_end; ++idx) {
    const std::vector<PFKFFlaggedElement>& secKFs =
    ...
    for (auto kf = KFbegin; kf != notmatched; ++kf) {
      const reco::PFBlockElementTrack* elemaskf = docast(const reco::PFBlockElementTrack*, kf->first);
      RO.secondaryKFs.emplace_back(elemaskf, true);
...
void PFEGammaAlgo::linkRefinableObjectECALToSingleLegConv(const pfEGHelpers::HeavyObjectCache* hoc, ProtoEGObject& RO) {
  ...
  for (auto& ecal : RO.ecalclusters) {
    ...
    for (auto kf = notconvkf; kf != notmatchedkf; ++kf) {
      float mvaval = evaluateSingleLegMVA(hoc, _currentblock, *cfg_.primaryVtx, kf->first->index());
...
void PFEGammaAlgo::linkRefinableObjectSecondaryKFsToECAL(ProtoEGObject& RO) {
  ...
  for (auto& skf : RO.secondaryKFs) {
    NotCloserToOther<reco::PFBlockElement::TRACK, reco::PFBlockElement::ECAL, false> TracksToECALwithCut(
        _currentblock, skf.first, 1.5f);
    auto notmatched = std::partition(ECALbegin, ECALend, TracksToECALwithCut);
    for (auto ecal = ECALbegin; ecal != notmatched; ++ecal) {
      const reco::PFBlockElementCluster* elemascluster = docast(const reco::PFBlockElementCluster*, ecal->first);
      if (addPFClusterToROSafe(elemascluster, RO)) {
        attachPSClusters(elemascluster, RO.ecal2ps[elemascluster]);
    ...
    for (const auto& secdkf : RO.secondaryKFs) {
      ...
      if (no_conv_ref) {
        ...
        float mvaval = (mvavalmapped != RO.singleLegConversionMvaMap.end()
                            ? mvavalmapped->second
                            : 3.0 + evaluateSingleLegMVA(hoc, _currentblock, *cfg_.primaryVtx, kf->index()));
...
float PFEGammaAlgo::calculateEleMVA(const pfEGHelpers::HeavyObjectCache* hoc,
  ...
  const double eneHcalGsf = std::accumulate(
      ro.hcalClusters.begin(), ro.hcalClusters.end(), 0.0, [](const double a, const PFClusterFlaggedElement& b) {
...
reco::SuperCluster PFEGammaAlgo::buildRefinedSuperCluster(const PFEGammaAlgo::ProtoEGObject& RO) {
  ...
  for (auto& clus : RO.ecalclusters) {
    ...
    clusptr = edm::refToPtr<reco::PFClusterCollection>(clus.first->clusterRef());
    ...
    if (isEE && RO.ecal2ps.count(clus.first)) {
      ...
      PS1_clus_sum = std::accumulate(
          psclusters.begin(), psclusters.end(), 0.0, std::bind(sumPSEnergy, _1, _2, reco::PFBlockElement::PS1));
      PS2_clus_sum = std::accumulate(
          psclusters.begin(), psclusters.end(), 0.0, std::bind(sumPSEnergy, _1, _2, reco::PFBlockElement::PS2));
      ...
      cluscalibe = cfg_.thePFEnergyCalibration->energyEm(
  ...
  clusptr = edm::refToPtr<reco::PFClusterCollection>(RO.ecalclusters.front().first->clusterRef());
  ...
  for (const auto& clus : RO.ecalclusters) {
    ...
    if (RO.ecal2ps.count(clus.first)) {
      ...
      for (const auto& pscluselem : cluspsassociation) {
        edm::Ptr<reco::PFCluster> psclus = edm::refToPtr<reco::PFClusterCollection>(pscluselem.first->clusterRef());
#ifdef PFFLOW_DEBUG
...
void PFEGammaAlgo::unlinkRefinableObjectKFandECALWithBadEoverP(ProtoEGObject& RO) {
  ...
  const double gsfOuterEta = RO.primaryGSFs.front().first->positionAtECALEntrance().Eta();
  double tot_ecal = 0.0;
  ...
  for (const auto& ecal : RO.ecalclusters) {
    ...
    for (const auto& brem : RO.brems) {
      const float dist =
          _currentblock->dist(brem.first->index(), ecal.first->index(), _currentlinks, reco::PFBlock::LINKTEST_ALL);
  ...
  for (auto secd_kf = RO.secondaryKFs.begin(); secd_kf != RO.secondaryKFs.end(); ++secd_kf) {
    ...
    for (auto ecal = RO.ecalclusters.begin(); ecal != RO.ecalclusters.end(); ++ecal) {
      ...
      const double detaGsf = std::abs(gsfOuterEta - ecal->first->clusterRef()->positionREP().Eta());
      const double detaBrem = std::abs(closest_brem_eta[bremidx] - ecal->first->clusterRef()->positionREP().Eta());

      ElementMap::value_type check_match(ecal->first, secd_kf->first);
      auto kf_matched = std::find(RO.localMap.begin(), RO.localMap.end(), check_match);

      const float tkdist = _currentblock->dist(
          secd_kf->first->index(), ecal->first->index(), _currentlinks, reco::PFBlock::LINKTEST_ALL);
      ...
      if (Epin > 3 && kf_matched != RO.localMap.end() && tkdist != -1.0f && tkdist < minbremdist && detaGsf > 0.05 &&
          detaBrem > 0.015) {
...
void PFEGammaAlgo::unlinkRefinableObjectKFandECALMatchedToHCAL(ProtoEGObject& RO,
  ...
  for (auto secd_kf = RO.secondaryKFs.begin(); secd_kf != RO.secondaryKFs.end(); ++secd_kf) {
    bool remove_this_kf = false;
    NotCloserToOther<reco::PFBlockElement::TRACK, reco::PFBlockElement::HCAL> tracksToHCALs(_currentblock,
                                                                                            secd_kf->first);
    reco::TrackRef trkRef = secd_kf->first->trackRef();
