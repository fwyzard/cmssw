...
PFProducer::PFProducer(const edm::ParameterSet& iConfig)
    : putToken_{produces<reco::PFCandidateCollection>()}, pfAlgo_(iConfig.getUntrackedParameter<bool>("debug", false)) {
  //--ab: get calibration factors for HF:
  auto thepfEnergyCalibrationHF =
      std::make_shared<PFEnergyCalibrationHF>(iConfig.getParameter<bool>("calibHF_use"),
  ...
  usePFElectrons_ = iConfig.getParameter<bool>("usePFElectrons");
  ...
  usePhotonReg_ = (usePFPhotons_) ? iConfig.getParameter<bool>("usePhotonReg") : false;

  useRegressionFromDB_ = (usePFPhotons_) ? iConfig.getParameter<bool>("useRegressionFromDB") : false;
  ...
  std::shared_ptr<PFSCEnergyCalibration> thePFSCEnergyCalibration(new PFSCEnergyCalibration(
      calibPFSCEle_Fbrem_barrel, calibPFSCEle_Fbrem_endcap, calibPFSCEle_barrel, calibPFSCEle_endcap));
  ...
  double nSigmaECAL = iConfig.getParameter<double>("pf_nsigma_ECAL");
  double nSigmaHCAL = iConfig.getParameter<double>("pf_nsigma_HCAL");

  //PFElectrons Configuration
  double mvaEleCut = iConfig.getParameter<double>("pf_electron_mvaCut");

  string mvaWeightFileEleID = iConfig.getParameter<string>("pf_electronID_mvaWeightFile");

  bool applyCrackCorrectionsForElectrons = iConfig.getParameter<bool>("pf_electronID_crackCorrection");
  ...
  edm::ParameterSet iCfgCandConnector = iConfig.getParameter<edm::ParameterSet>("iCfgCandConnector");
  ...
  pfAlgo_.setParameters(nSigmaECAL, nSigmaHCAL, calibration, thepfEnergyCalibrationHF);
...
void PFProducer::beginRun(const edm::Run& run, const edm::EventSetup& es) {
  ...
  */
  ...
  if (usePFPhotons_) {
    ...
    pfAlgo_.setPFPhotonRegWeights(
        ReaderLCEB_, ReaderLCEE_, ReaderGCBarrel_, ReaderGCEndCapHighr9_, ReaderGCEndCapLowr9_, ReaderEcalRes_);
  }
}
...
void PFProducer::produce(Event& iEvent, const EventSetup& iSetup) {
  ...
  if (useEGammaElectrons_)
    pfAlgo_.setEGElectronCollection(iEvent.get(inputTagEgammaElectrons_));
  ...
  if (usePFElectrons_) {
    std::unique_ptr<reco::PFCandidateElectronExtraCollection> pOutputElectronCandidateExtraCollection(
        pfAlgo_.transferElectronExtra());
  ...
  if (usePFPhotons_) {
    std::unique_ptr<reco::PFCandidatePhotonExtraCollection> pOutputPhotonCandidateExtraCollection(
        pfAlgo_.transferPhotonExtra());
  ...
  std::unique_ptr<reco::PFCandidateCollection> pCosmicsMuonCleanedCandidateCollection(
      pfAlgo_.getPFMuonAlgo()->transferCleanedCosmicCandidates());
  // Save tracker/global cleaned muon candidates
  std::unique_ptr<reco::PFCandidateCollection> pTrackerAndGlobalCleanedMuonCandidateCollection(
      pfAlgo_.getPFMuonAlgo()->transferCleanedTrackerAndGlobalCandidates());
  // Save fake cleaned muon candidates
  std::unique_ptr<reco::PFCandidateCollection> pFakeCleanedMuonCandidateCollection(
      pfAlgo_.getPFMuonAlgo()->transferCleanedFakeCandidates());
  // Save punch-through cleaned muon candidates
  std::unique_ptr<reco::PFCandidateCollection> pPunchThroughMuonCleanedCandidateCollection(
      pfAlgo_.getPFMuonAlgo()->transferPunchThroughCleanedMuonCandidates());
  // Save punch-through cleaned neutral hadron candidates
  std::unique_ptr<reco::PFCandidateCollection> pPunchThroughHadronCleanedCandidateCollection(
      pfAlgo_.getPFMuonAlgo()->transferPunchThroughCleanedHadronCandidates());
  // Save added muon candidates
  std::unique_ptr<reco::PFCandidateCollection> pAddedMuonCandidateCollection(
      pfAlgo_.getPFMuonAlgo()->transferAddedMuonCandidates());
  ...
  if (usePFElectrons_) {
    std::unique_ptr<reco::PFCandidateCollection> pOutputElectronCandidateCollection(
        pfAlgo_.transferElectronCandidates());
