  ...
  for (const auto& blockref : otherBlockRefs) {
    ...
    if (!pfeg_->getCandidates().empty()) {
      LOGDRESSED("PFEGammaProducer") << "Block with " << elements.size() << " elements produced "
                                     << pfeg_->getCandidates().size() << " e-g candidates!" << std::endl;
    }

    const size_t egsize = egCandidates_->size();
    egCandidates_->resize(egsize + pfeg_->getCandidates().size());
    reco::PFCandidateCollection::iterator eginsertfrom = egCandidates_->begin() + egsize;
    std::move(pfeg_->getCandidates().begin(), pfeg_->getCandidates().end(), eginsertfrom);

    const size_t egxsize = egExtra_->size();
    egExtra_->resize(egxsize + pfeg_->getEGExtra().size());
    reco::PFCandidateEGammaExtraCollection::iterator egxinsertfrom = egExtra_->begin() + egxsize;
    std::move(pfeg_->getEGExtra().begin(), pfeg_->getEGExtra().end(), egxinsertfrom);

    const size_t rscsize = sClusters_->size();
    sClusters_->resize(rscsize + pfeg_->getRefinedSCs().size());
    reco::SuperClusterCollection::iterator rscinsertfrom = sClusters_->begin() + rscsize;
    std::move(pfeg_->getRefinedSCs().begin(), pfeg_->getRefinedSCs().end(), rscinsertfrom);
  }
